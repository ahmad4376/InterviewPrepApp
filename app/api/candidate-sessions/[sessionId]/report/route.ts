...
export async function GET(
  ...
  doc.fontSize(16).fillColor("#111827").text("Interview Details", 70, boxY + 15);
  ...
  feedback.categories.forEach((cat) => {
    ...
    doc.fontSize(13).fillColor("#1e293b").text(cat.name, 65, catY + 12);
    doc.fontSize(13).fillColor(getScoreColor(cat.score)).text(`${cat.score}/5`, 480, catY + 12);
    const barY = catY + 32;
    doc.rect(65, barY, 460, 6).fill("#e5e7eb");
    doc.rect(65, barY, 460 * (cat.score / 5), 6).fill(getScoreColor(cat.score));
    doc.fontSize(10).fillColor("#475569").text(cat.feedback, 65, barY + 14, { width: 455 });
    doc.y = catY + catH + 12;
  });
  ...
  if (feedback.questionFeedback && feedback.questionFeedback.length > 0) {
    ...
    feedback.questionFeedback.forEach((qf, index) => {
      ...
      const qY = doc.y;
      doc.fontSize(12).fillColor("#1e40af").text(`Question ${index + 1}`, 50, qY);
      ...
      doc.fontSize(10).fillColor("#334155").text(qf.question, 60, questionBoxY2 + 8, { width: 480 });
      ...
      doc.fontSize(10).fillColor("#166534").text(qf.assessment, 60, aBoxY + 8, { width: 480 });
      doc.y = aBoxY + aH + 16;
    });
  }
